...
class _RemederTaskItemState extends State<RemederTaskItem> {
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Icon(Icons.alarm,color: isSwitched ? AppColors.primary : AppColors.notactive, size: 18),
  ...
  void toggleSwitch(bool value) {
    setState(() {
      ...
      if (!value) {
        ...
        DateTime fullDateTime = DateTime.fromMillisecondsSinceEpoch(widget.taskModel.date);
  ...
  //   List<int> repeatDays = List.from(task.repeatDays); // نسخة قابلة للتعديل من أيام التكرار
  //
  //   showDialog(
  //     context: context,
  //     builder: (context) {
  ...
  //                       String dayName = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][index];
  ...
  //                           decoration: BoxDecoration(
  //                             color: repeatDays.contains(index) ? Colors.blue : Colors.grey.shade300,
  //                             borderRadius: BorderRadius.circular(8),
  //                           ),
  //                           child: Text(
  //                             dayName,
  //                             style: TextStyle(
  //                               color: repeatDays.contains(index) ? Colors.white : Colors.black,
  //                             ),
  //                           ),
  //                         ),
  //                       );
  ...
  //                     SnackBar(content: Text(" ✅ تم إعادة جدولة الإشعارات للمهمة")),
  //                   );
  //                 },
  //                 child: Text("Save"),
  //               ),
    ...
    showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: Text("Edit Task",style: TextStyle(fontSize: 24,fontWeight: FontWeight.bold),),
              content: SingleChildScrollView(
                child: Form(
                  key: _formKey,
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 10),
                      ListTile(
                        title: Text("Date: ${selectedDate.toLocal()}".split(' ')[0],style: TextStyle(fontSize: 22,fontWeight: FontWeight.w700),),
                        trailing: Icon(Icons.calendar_today),
                        onTap: () async {
                          DateTime? pickedDate = await showDatePicker(
                            context: context,
                      ...
                      const SizedBox(height: 10),
                      ListTile(
                        title: Text("Time: ${timeController.text}",style: TextStyle(fontSize: 21,fontWeight: FontWeight.w700),),
                        trailing: Icon(Icons.access_time),
                        onTap: () async {
                          TimeOfDay? pickedTime = await showTimePicker(
                            context: context,
                      ...
                      Text("Repeat on:", style: TextStyle(fontWeight: FontWeight.bold,fontSize: 18)),
                      Wrap(
                        spacing: 8.0,
                        children: List.generate(7, (index) {
                          String dayName = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][index];
                          return GestureDetector(
                            ...
                            child: Container(
                              ...
                              padding: EdgeInsets.symmetric(vertical: 8, horizontal: 12),
                              decoration: BoxDecoration(
                                color: repeatDays.contains(index) ? Colors.green : Colors.grey.shade300,
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Text(
                                dayName,
